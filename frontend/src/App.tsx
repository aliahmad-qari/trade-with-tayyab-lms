          ...
          <div className="flex-grow flex flex-col">
            ...
            {activeTab === "home" && (
              <div className="space-y-16 animate-in fade-in duration-300">
                ...
                {/* FEATURED COURSE MARKETPLACE */}
                <div className="space-y-10">
                  <div className="text-center space-y-2">
                    <span className="text-[10px] text-brand-violet font-mono uppercase font-bold tracking-widest block">Featured Masterclasses</span>
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-white">Course Marketplace</h2>
                    <p className="text-xs text-gray-450 max-w-lg mx-auto font-sans">Discover professional smart money trading classes published live by Tayyab.</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {courses.filter(c => c.isPublished !== false).slice(0, 3).map((course) => {
                      const isPurchased = enrolledCourseIds.includes(course.id);
                      return (
                        <div key={course.id} className="rounded-xl overflow-hidden glass-panel border border-white/5 flex flex-col justify-between group h-full bg-brand-card">
                          <div className="relative aspect-video overflow-hidden">
                            ...
                            />
                            <div className="absolute top-2.5 left-2.5 px-2 py-1 bg-black/80 backdrop-blur-md rounded text-[9px] font-bold uppercase border border-brand-purple/30 text-brand-purple">
                              📈 {course.category}
                            </div>
                          </div>
                          ...
                          <div className="p-4 flex-grow flex flex-col justify-between space-y-3">
                            ...
                            <div className="space-y-3 pt-2 text-left">
                              <div className="flex items-center gap-1.5">
                                <span className="text-xs font-bold text-emerald-400">★ {course.rating}</span>
                                <span className="text-[10px] text-gray-500">({course.reviewsCount || 1} assessments)</span>
                              </div>

                              <div className="border-t border-white/5 pt-3.5 flex items-center justify-between">
                                <div className="text-left">
                                  <span className="text-[9px] text-gray-550 block font-mono">FEE</span>
